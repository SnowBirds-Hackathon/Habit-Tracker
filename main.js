...
function displayTodo() {
  ...
  todoArray.forEach((list, ind) => {
    ...
    <div class='w-1/3 flex items-center justify-start border border-grey-500 rounded-full h-auto'>
        <p class='w-2/3 text-gray-600 m-1 line-clamp-1 flex-wrap overflow-hidden hover:overflow-visible hover:break-inside-auto hover:line-clamp-4 font-excalidraw text-lg pl-2' >${
          list.habit
        }</p>
    ...
    <div class='w-2/3 flex gap-x-2 justify-around items-center'>
        <input id="sunday${ind}" ${list.days.sunday ? 'checked' : ''} onclick='toggleDay(${ind})' type="checkbox" value="sunday" class="h-8 w-8 m-1 rounded-full">
        ...
        <input id="tuesday${ind}" ${list.days.tuesday ? 'checked' : ''} onclick='toggleDay(${ind})' type="checkbox" value="tuesday" class="h-8 w-8 m-1 rounded-full">
        <input id="wednesday${ind}" ${list.days.wednesday ? 'checked' : ''} onclick='toggleDay(${ind})' type="checkbox" value="wednesday" class="h-8 w-8 m-1 rounded-full">
        <input id="thursday${ind}" ${list.days.thursday ? 'checked' : ''} onclick='toggleDay(${ind})' type="checkbox" value="thursday" class="h-8 w-8 m-1 rounded-full">
        <input id="friday${ind}" ${list.days.friday ? 'checked' : ''} onclick='toggleDay(${ind})' type="checkbox" value="friday" class="h-8 w-8 m-1 rounded-full ml-2">
        <input id="saturday${ind}" ${list.days.saturday ? 'checked' : ''} onclick='toggleDay(${ind})' type="checkbox" value="saturday" class="h-8 w-8 m-1 rounded-full">
    </div>    
   </div>`
  })
